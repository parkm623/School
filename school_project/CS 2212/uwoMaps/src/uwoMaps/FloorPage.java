 ...
 * @since 1.0
 * 
 *        This class represents the floorpage, which the user gets taken to
 *        after the pick a floor number on a building in the buildingPage. From
 *        here, you are able to add custom POIs, add POIs to your favourites,
 *        and list all the POIs on the floor in question
 * 
 *        <p>
 *        This class is set up dynamically so it will open the correct blueprint
 *        in the correct building, while still keeping the functionality
 *        <p>
 */
...
public class FloorPage extends JFrame implements ActionListener {
	...
	/*
	 * Check variable to see if certain labels should be displayed, if a user clicks
	 * on the map, this gets set to true
	 */
	...
	/*
	 * Label that will only show up if the POI has been successfully added to
	 * favourites list
	 */
	...
	public FloorPage(Building build, Floor f) {
		...
		accessImg = new ImageIcon(
				accessImg.getImage().getScaledInstance(getWidth() - 300, getHeight(), Image.SCALE_SMOOTH));

		washroomImg = new ImageIcon(getClass().getResource(f.washroomViewURL));
		washroomImg = new ImageIcon(
				washroomImg.getImage().getScaledInstance(getWidth() - 300, getHeight(), Image.SCALE_SMOOTH));

		plainImg = new ImageIcon(getClass().getResource(f.blankViewURL));
		plainImg = new ImageIcon(
				plainImg.getImage().getScaledInstance(getWidth() - 300, getHeight(), Image.SCALE_SMOOTH));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == submitCustomButton) {
			// Drop a purple pin where the custom POI was clicked, and add that point to
			// POIs
	...
	/*
	 * Action listener which is added to the blueprint, if it is clicked, drop a red
	 * pin where the click happened
	 */
